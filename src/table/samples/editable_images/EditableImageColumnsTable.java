package table.samples.editable_images;
...
public class EditableImageColumnsTable extends DefaultTable<Person> {

	protected PersonList list;
	private static final Image CHECK;
	private static final Image UNCHECK;

	static {
		CHECK = ImageDescriptor.createFromFile(
				EditableImageColumnsTable.class, "check.png")
				.createImage();
		UNCHECK = ImageDescriptor.createFromFile(
				EditableImageColumnsTable.class, "uncheck.png")
				.createImage();
	}

	public EditableImageColumnsTable(Composite parent, int style,
			PersonList list) {
		super(parent, style);
	...
	protected DefaultColumn<Person>[] getColumns() {
		return new BaseColumn[] { new NameColumn(), new AddressColumn(),
				new OtherColumn(), new ActiveColumn() };
	}
	...
	class NameColumn extends BaseColumn {
		public NameColumn() {
			super("name", "Person's Name", 200, SWT.LEFT, true,
	...
	class AddressColumn extends BaseColumn {
		public AddressColumn() {
			super("address", "Address", 150, SWT.LEFT, true,
					new TextCellEditor(table));
		}
	...
	class OtherColumn extends BaseColumn {
		public OtherColumn() {
			super("other", "Other Information", 150, SWT.LEFT, true,
					new TextCellEditor(table));
		}
	...
	class ActiveColumn extends BaseColumn {
		public ActiveColumn() {
			super("active", "Active", 80, SWT.CENTER, true,
		...
		@Override
		public String getText(Person element) {
			return ""; // Nothing because we're only showing an image
		}
		
		@Override
		public Image getImage(Person element) {
			return element.isActive() ? CHECK : UNCHECK;
		}
